...
def search_radiko_programs(keyword: str, auth_token: str, page: int = 1) -> SearchResponse:
    ...
    try:
        ...
        if e.response.status_code == 401:
            raise HTTPException(status_code=401, detail="Radiko API authentication failed. Token might be expired.")
        else:
            raise HTTPException(status_code=502, detail=f"Radiko API returned an error: {e.response.status_code}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to connect to Radiko API: {e}")
    ...
    for prog in data.get('data', []):
        try:
            ...
            programs.append(SearchResult(
                title=prog.get('title', '不明'),
                station_id=station_id,
                station_name=station_name,
                start_time=JST.localize(datetime.strptime(start_time_str, '%Y-%m-%d %H:%M:%S')),
                end_time=JST.localize(datetime.strptime(end_time_str, '%Y-%m-%d %H:%M:%S')),
                pfm=prog.get('performer'),
                image_url=prog.get('img')
            ))
        except (ValueError, TypeError) as e:
            # 日付フォーマットエラーなど、個別の番組データの問題はスキップ
            print(f"警告: 番組データの解析に失敗しました。スキップします。 Error: {e}, Data: {prog}")
            continue
...
def start_download_job(job_id, station_id, station_name, program_title, start_time_str, end_time_str,
    ...
    try:
        ...
        command = ['ffmpeg', '-loglevel', 'error', '-headers', headers_str, '-i', stream_url, '-acodec', 'copy',
                   output_path]
...
def search_programs(keyword: str, page: int = Query(1, ge=1), x_radiko_authtoken: str = Header(...),
                    current_user: str = Depends(get_current_user)):
...
def schedule_download(request: DownloadRequest, current_user: str = Depends(get_current_user)):
    ...
    scheduler.add_job(
        ...
        args=[job_id, request.station_id, request.station_name, request.program_title, request.start_time,
              request.end_time, request.radiko_token] # radiko_tokenを渡す
    )
